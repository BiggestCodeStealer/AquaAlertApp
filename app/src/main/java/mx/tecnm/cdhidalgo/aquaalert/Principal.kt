...
class Principal : AppCompatActivity(), ItemListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(supportActionBar != null) {
            supportActionBar!!.title = "Recordatorios"
        }
        ...
        btnAdd.setOnClickListener {
            startActivity(Intent(this, Recordatorio::class.java))
        }
    ...
    private fun fill(){
        ...
        val peticion = object: JsonObjectRequest(Request.Method.GET, url, null, {
            ...
            response ->
                ...
                lista = Array(data.length()){arrayOfNulls<String>(6)}
                ...
                for(i in 0 until data.length()){
                    ...
                    lista[i][2]  = data.getJSONObject(i).getString("desc")
                    lista[i][3]  = data.getJSONObject(i).getString("dose")
                    lista[i][4]  = data.getJSONObject(i).getString("hour")
                }
    ...
    override fun onClick(v: View?, position: Int) {
        ...
        Toast.makeText(this, "Click en posicion:$position, id:${lista[position][0]}", Toast.LENGTH_SHORT).show()
    }
    ...
    override fun onEdit(v: View?, position: Int) {
        ...
        intent.putExtra("desc", lista[position][2])
        intent.putExtra("dose", lista[position][3])
        intent.putExtra("hour", lista[position][4])
    ...
    override fun onDel(v: View?, position: Int) {
        ...
        AlertDialog.Builder(this)
            .setTitle("Eliminar")
            .setMessage("¿Desea eliminar el recordatorio ${lista[position][1]}?")
            .setPositiveButton("Sí"){dialog, which -> //si se da click en si se elimina el sensor seleccionado
                //se crea la url para eliminar el sensor seleccionado
                val url = Uri.parse(Config.URL + "recordatorios/" + lista[position][0])
                    .buildUpon()
                    .build().toString()
                ...
                val peticion = object: StringRequest(Method.DELETE, url, {
    ...
    override fun onBackPressed() {
        // Borrar el token JWT de las preferencias compartidas
        with(sesion.edit()) {
            remove("jwt")
            apply()
        }
